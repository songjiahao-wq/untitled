...
def ghostnetv2(**kwargs):
    cfgs = [
        ...
        [[3, 48, 24, 0, 2]], #1-p2/4
        [[3, 72, 24, 0, 1]],
        [[5, 72, 40, 0.25, 2]], # 3-p3/8
        [[5, 120, 40, 0.25, 1]],
        [[3, 240, 80, 0, 2]], #5-p4/16
        [[3, 200, 80, 0, 1],
         [3, 184, 80, 0, 1],
         [3, 184, 80, 0, 1],
         [3, 480, 112, 0.25, 1],
...
class ghostnetv2_model(nn.Module):
    def __init__(self,c1 ,c2, layer=0):
        super().__init__()
        model = ghostnetv2()
        if layer ==3:
            ...
            self.model = nn.Sequential(*list(model.children())[3][5:7])
        elif layer == 5:
            self.model = nn.Sequential(*list(model.children())[3][7:])
    def forward(self, x):
        out = self.model(x)
        return out

if __name__ == "__main__":
    mode = ghostnetv2_model(3,240,3)
    mode2 = ghostnetv2_model(3,240,4)
    mode3 = ghostnetv2_model(3,240,5)


    model = ghostnetv2()
    model.eval()
    input = torch.randn(1, 3, 640, 640) #输入
    # print(mode(input).shape)
    # input = mode(input)
    # print(mode2(input).shape)
    # input = mode2(input)
    print(model)
    model1 = nn.Sequential(*list(model.children())[:3])
    model2 = nn.Sequential(*list(model.children())[3][0])
    model3 = nn.Sequential(*list(model.children())[3][1:3])
    model4 = nn.Sequential(*list(model.children())[3][3:5])
    model5 = nn.Sequential(*list(model.children())[3][5:7])
    model6 = nn.Sequential(*list(model.children())[3][7:])
    print(input.shape)
    print(model1(input).shape)
    input = model1(input)
    print(model2(input).shape)
    input = model2(input)
    print(model3(input).shape)
    input = model3(input)
    print(model4(input).shape)
    input = model4(input)
    print(model5(input).shape)
    input = model5(input)
    print(model6(input).shape)
